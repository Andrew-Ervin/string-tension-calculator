// String Tension Calculator Constants

export const PLAIN_UNIT_WEIGHTS: Record<number, number> = {
  0.007: 0.00001085, 0.008: 0.00001418, 0.0085: 0.00001601,
  0.009: 0.00001794, 0.0095: 0.00001999, 0.010: 0.00002215,
  0.0105: 0.00002442, 0.011: 0.00002680, 0.0115: 0.00002930,
  0.012: 0.00003190, 0.013: 0.00003744, 0.0135: 0.00004037,
  0.014: 0.00004342, 0.015: 0.00004984, 0.016: 0.00005671,
  0.017: 0.00006402, 0.018: 0.00007177, 0.019: 0.00007997,
  0.020: 0.00008861, 0.022: 0.00010722, 0.024: 0.00012760,
  0.026: 0.00014975,
};

export const WOUND_UNIT_WEIGHTS: Record<number, number> = {
  0.017: 0.00005524,
  0.018: 0.00006215,
  0.019: 0.00006947,
  0.020: 0.00007495,
  0.021: 0.00008293,
  0.022: 0.00009184,
  0.024: 0.00010857,
  0.026: 0.00012671,
  0.028: 0.00014666,
  0.030: 0.00017236,
  0.032: 0.00019347,
  0.034: 0.00021590,
  0.036: 0.00023964,
  0.038: 0.00026471,
  0.039: 0.00027932,
  0.042: 0.00032279,
  0.044: 0.00035182,
  0.046: 0.00038216,
  0.048: 0.00041382,
  0.049: 0.00043014,
  0.052: 0.00048109,
  0.054: 0.00053838,
  0.056: 0.00057598,
  0.059: 0.00064191,
  0.060: 0.00066542,
  0.062: 0.00070697,
  0.064: 0.00074984,
  0.066: 0.00079889,
  0.068: 0.00084614,
  0.070: 0.00089304,
  0.072: 0.00094124,
  0.074: 0.00098869,
  0.080: 0.00115011,
};

export const PLAIN_GAUGES = Object.keys(PLAIN_UNIT_WEIGHTS)
  .map(Number)
  .sort((a, b) => a - b);

export const WOUND_GAUGES = Object.keys(WOUND_UNIT_WEIGHTS)
  .map(Number)
  .sort((a, b) => a - b);

export const A4 = 440.0;

export const NOTE_INDEX: Record<string, number> = {
  'C': -9, 'C#': -8, 'D': -7, 'D#': -6,
  'E': -5, 'F': -4, 'F#': -3,
  'G': -2, 'G#': -1, 'A': 0,
  'A#': 1, 'B': 2,
};

export const FLAT_TO_SHARP: Record<string, string> = {
  'Db': 'C#', 'Eb': 'D#', 'Gb': 'F#', 'Ab': 'G#', 'Bb': 'A#',
};

export const DEFAULT_TARGET_PLAIN: [number, number] = [13.0, 15.5];
export const DEFAULT_TARGET_WOUND: [number, number] = [16.0, 20.0];
